...
class GraphDataset(data.Dataset):
    ...
    def __init__(self, root, ids, target_patch_size=-1):
        ...
        """
        self.root = root
        print('root', self.root) # troubleshooting
        self.ids = ids
        #self.target_patch_size = target_patch_size
        #self.classdict = {'normal': 0, 'luad': 1, 'lscc': 2}        #
        self.classdict = {'normal': 0, 'mild': 1, 'severe': 2}        # our project
    ...
    def __getitem__(self, index):
        ...
        #print('info', info)
        file_name, label = info.split('\t')[0].rsplit('.', 1)[0], info.split('\t')[1] #index out of range error
        #print('file_name' , file_name, 'label', label)
        site, file_name = file_name.split('-') # troubleshooting, replacing '/' with '-'
        #print('site', site, 'file_name', file_name)

        # For THREE classes version in our_project
        """
        if site =='THREE':
            print('site == THREE')
            file_path = self.root + 'THREE_class/simclr_files' # commenting this out, because might be unnecessary
        """
        #TROUBLESHOOTING HERE#####file_path = self.root + 'OUR_PROJ_{}_features/simclr_files'.format(site)  
        file_path = self.root.format(site)   ## attempted fix
