...
def main(args):
   ...
   site, file_name = file_name.split('-') #modifying for the way formatted in this project
   ...
   print('p', p)
   file_path = os.path.join(args.path_patches, '{}_files/1.0/'.format(file_name)) #20.0/'.format(file_name)) # trying to address a path reading error
   #print('file_path', file_path)
   #print('file_name', file_name)
   print('both', file_path + file_name)

   ### checking for why path doesn't exist for openslide 
   """
   file_path = os.path.join(args.path_WSI, '{}.svs').format(file_name)
   print('file_path', file_path)
   #print('file_name', file_name)
   #print('both', file_path + file_name)
   ...
   """
   ### 
   ori = openslide.OpenSlide(os.path.join(args.path_WSI, '{}.tiff').format(file_name)) ### .tiff seems to have fixed the error here.... testing if this will fix not found error '{}.svs').format(file_name))
   ...
   assign_matrix = torch.load('graphcam/s_matrix_ori.pt') # doesn't seem to be doing the visualization..
   m = nn.Softmax(dim=1)
